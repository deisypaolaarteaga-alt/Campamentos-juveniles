...
export default function RegistroPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error, session } = await registro(email, password, nombre);
    if (error) {
      toast.error(error.message);
    } else if (session) {
      toast.success('¡Cuenta creada! Configura tu Bosque');
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      >
        <h2 className="font-montserrat font-bold text-xl mb-6" style={{ color: '#1A1A2E' }}>
          Crear cuenta
        </h2>
